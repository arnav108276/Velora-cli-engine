...
```bash
# Check cluster
aws eks describe-cluster --name arnav-velora1 --region ap-south-1
...
# Should show: arnav-velora1.ap-south-1.eksctl.io
```
...
```bash
aws eks update-kubeconfig --name arnav-velora1 --region ap-south-1
```
...
velora create test-api \
  --type api \
  --description "Test API Service" \
  --location /tmp/test-api
```
...
🔍 Checking EKS cluster: arnav-velora1...
...
```bash
eksctl scale nodegroup --cluster=arnav-velora1 --nodes=0 velora-workers
```

2. **Delete cluster completely**:
```bash
eksctl delete cluster --name arnav-velora1 --region ap-south-1
```
